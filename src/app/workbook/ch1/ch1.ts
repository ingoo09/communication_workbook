...
export const chapter = {
  id: '1-introduction-basic-tools',
  title: 'Chapter 1. Introduction and Basic Tools',
  sections: [

    { //문제 1
      id: '1-1',
      title: '1. 온라인 교재 사용 방법 익히기',
      problems: [
        {
          id: '1-1A',
          title: '1.A.',
          type: 'essay',
          prompt: `이 문제를 통해 온라인 교재의 기본 학습 흐름을 익혀보자.

**문제. 통신 시스템에서 송신기와 수신기의 역할을 각각 간단히 설명하시오.**

답안을 작성한 뒤 아래 순서대로 기능을 사용해보시오.

1. [저장] 버튼을 눌러 답안을 저장한다.
2. [내 답안 채점하기] 버튼을 눌러 채점 결과와 피드백을 확인한다.
3. 피드백을 참고하여 필요한 경우 답안을 수정한다.
4. 수정한 답안을 다시 저장하고 채점한다.
5. 채점 결과가 기준 점수 이상이면 [정답 및 풀이 보기]를 눌러 자신의 답안과 비교한다.`,
          referenceAnswer: `송신기는 전달하고자 하는 정보를 통신 채널을 통해 전송할 수 있는 신호 형태로 변환하여 보내는 역할을 한다.

수신기는 통신 채널을 통해 전달된 신호를 받아 원래의 정보를 복원하는 역할을 한다.`,
        },
        {
          id: '1-1B',
          title: '1.B.',
...
>>> print("Hello World!")
\`\`\`
각 명령을 실행한 후, Console에 어떤 결과가 나타났는지 간단히 작성하시오.

※ Console형 문제에서는 명령을 한 줄씩 직접 입력하여 실행할 수 있으며, 키보드의 위/아래 방향키(↑/↓)로 이전에 입력한 명령을 다시 불러올 수 있다.`,
          referenceAnswer: `각 명령의 결과는 다음과 같다.

1. \`2+3\` → 5
2. \`A=10\` → 변수 A에 10을 저장하며 별도의 출력은 없다.
3. \`A*2\` → 20
4. \`print("Hello World!")\` → Hello World!

Console형 문제에서는 명령을 한 줄씩 직접 입력하고 바로 실행 결과를 확인할 수 있다.`,
        },
        {
          id: '1-1C',
          title: '1.C.',
...
Hello World!
\`\`\`
\`print()\` 함수는 괄호 안의 문자열이나 값을 화면에 출력하는 함수이다.`,
        },
        {
          id: '1-1D',
          title: '1.D.',
...
print(x+y)
\`\`\`
코드 실행 후, 아래 명령을 Console에 한 줄씩 입력하여 결과를 확인하시오.
\`\`\`python
>>> x
>>> y
>>> x*y
...
x/y    -> 0.5
\`\`\`
Python + Console 결합형에서는 먼저 실행한 Python 스크립트의 변수와 실행 환경을 이어서 Console에서 사용할 수 있다.`,
        },
        {
          id: '1-1E',
          title: '1.E.',
          type: 'proof',
          prompt: `증명형 문제 사용 방법을 익혀보자.

**문제. 오일러 공식(Euler's formula)을 쓰시오.**

증명형 문제에서는 다음 방법 중 편한 방법을 사용하여 답안을 작성할 수 있다.

1. 화면에서 직접 손글씨로 작성 후 '필기 인식' 클릭
2. 한글/영문과 수식을 직접 한글 또는 Word로 타이핑한 뒤 캡처하여 업로드
3. 종이에 작성한 답안을 촬영한 뒤 업로드
4. LaTeX 입력에 익숙하다면 직접 LaTeX 형식으로 작성

위 방법 중 하나를 이용하여 작성하시오.`,
          referenceAnswer: `오일러 공식은 다음과 같다.
$$
e^{j\\theta}=\\cos\\theta+j\\sin\\theta
$$
수학 분야에서는 허수 단위를 \(i\)로 써서
$$
...
와 같이 나타내기도 한다.`,
        },
        {
          ...
          graphYAxisLabel: 'y',
          graphShowGrid: true,
          prompt: `그래프형 문제 사용 방법을 익혀보자.

그래프형 문제에서는 화면의 좌표축 위에 직접 그래프를 그리거나, 종이에 그린 그래프를 촬영하여 이미지로 업로드할 수 있다.

아래 함수의 그래프를 $-3\\leq x\\leq3$ 범위에서 직접 그리시오.
$$
y=x
$$
- 직접 그리기에서는 펜, 부분 지우개, 실행 취소, 모두 지우기 기능을 사용할 수 있다.
- 필요하면 격자를 켜거나 끌 수 있다.
- 그래프를 직접 그리는 대신 이미지 파일을 업로드해도 된다.
- 그래프 설명란에는 그래프의 특징을 간단히 적어도 된다.`,
          referenceAnswer: `정답 그래프는 원점 $(0,0)$을 지나고 기울기가 1인 직선이다.

따라서 $-3\\leq x\\leq3$ 범위에서 대표적으로
$(-3,-3)$, $(0,0)$, $(3,3)$을 지나며, 왼쪽 아래에서 오른쪽 위로 증가하는 직선 형태가 되어야 한다.

채점에서는 직선이 원점을 지나고 기울기 1의 형태를 올바르게 나타냈는지를 중심으로 확인한다.`,
        },
      ],
    },
    { //문제 2
      id: '1-2',
      title: '2. Python의 다양한 변수 연산 및 그래프 출력',
      problems: [
        { //문제 2.A
          id: '1-2A',
          type: 'console',
...
`
        },
        {
          ...
          type: 'python',
          title: '3.B.',
          prompt: `문제 3.A에서 작성한 py 스크립트를 참고하여 10개의 사인 파형을 그리는 파일을 작성해보자.
...
`
        ...
        { //문제 3.D
          ...
          "type": "python",
          prompt: `
아래는 논리연산자를 이용하여 [[equation:1.1]]의 불연속 함수 $y(t)$를 그리는 py 스크립트이다.
$$
y(t)=
\\begin{cases}
...
plt.axis([-1,6,-2,2])
\`\`\`
코드를 실행하면 $1\\le t\\le2$ 구간에서는 $\\sin\\left(2\\pi\\times5t+\\frac{\\pi}{3}\\right)$가 나타나고, 그 외 구간에서는 0이 된다. 따라서, [[equation:1.1]]의 불연속 함수가 문제 의도에 맞게 그래프로 출력된다.
`
        },
        { //문제 3.E
          id: '1-3E',
...
`
    ...
    { //문제 4
      id: '1-4',
      title: '4. 사용자 함수(Function) 만들고 사용하기',
      problems: [
        {
          id: '1-4A',
          title: '4.A.',
...
`      
        },
        {
          ...
          "consoleEnabled": true,
          title: '4.C2.',
          prompt: `문제 4.C1에서 완성한 py 스크립트를 복사해서 붙여넣으시오. py 스크립트를 실행한 후, Console에서 아래 명령어를 실행하여, 본인이 작성한 함수가 제대로 동작하는지 확인하시오.
\`\`\`python
...
`
        },
        {
          ...
          title: '5.B.',
          prompt:
            `문제 5.A를 참고하여, sympy 모듈로 아래 등식이 성립하는지 판단하시오. Console 창에서 sympy 모듈로 각각을 계산한 코드를 보이시오.`
        },
        {
          id: '1-5B1',
          title: '5.B1.',
