...
[[equation:15.1]]의 첫 번째 줄에서 $m+k\\sigma$를 $t$로 치환하고, 우변과 좌변을 바꾸면 [[equation:15.2]]로 표현할 수 있다. 즉, 가우시안 확률 변수 $X$가 어떤 실수 $t$보다 높을 확률 $\\Pr\\{X \\ge t\\}$를 Q-function으로 표현할 수 있다. 빈칸을 채워 관계식을 완성하시오.
$$
\\Pr\\{X \\ge t\\}
=
Q(\\ ?\\ ),
...
`,
        ...
        { //문제 1.B
          ...
          "title": "1.B.",
          "type": "python",
          "prompt": `Q-function은 [[equation:15.3]]과 같이 에러 함수 ‘erfc()’와의 관계가 성립한다. 에러 함수는 scipy.special 모듈에 함수 ‘erfc()’로 정의되어 있으므로, [[equation:15.3]]의 우변을 이용하여 Q-function을 계산하는 것이 가능하다.
$$
Q(x)=\\frac{1}{2}\\operatorname{erfc}\\left(\\frac{x}{\\sqrt{2}}\\right)
\\qquad \\text{(식 15.3)}
$$          
아래는 함수 ‘erfc()’와 [[equation:15.3]]을 이용하여 $Q(x)$를 계산하는 py 스크립트이다. 아래 py 스크립트에서는 $Q(x)$의 매우 작은 범위까지 정확하게 확인하기 위해, ‘plt.plot()’를 사용하지 않고, y축을 로그 스케일로 출력하는 ‘plt.semilogy()’를 사용한다. ?를 채워 py 스크립트를 완성하고, 벡터 ‘x=np.arange(0,6,0.01)’의 각 원소에 대하여 $Q(x)$를 구하고, 결과 그래프를 확인하시오.
\`\`\`python
import numpy as np
...
`       
        ...
        { //문제 1.C
          ...
          "title": "1.C.",
          "type": "essay",
          "prompt": `[[equation:15.2]]와 [[equation:15.3]]을 이용하여, [[equation:15.4]]를 완성하시오.
$$
\\Pr\\{X\\ge t\\}
=
...
`
        ...
        { //문제 1.D
          "id": "15-1D",
          "title": "1.D.",
          "prompt": `[[equation:15.4]]를 이용하여 다양한 가우시안 확률 변수의 대소 확률을 Python에서 계산할 수 있다. Python의 ‘erfc()’를 사용하여 다음을 구하시오.`
        },
        {
          "id": "15-1D1",
          "title": "1.D1.",
...
[[equation:15.6]]을 증명하기 위해 아래 식의 ?를 채우시오.
$$
\\sigma_Y^2
=
...
`
        },
        {
          ...
          "title": "2.D2.",
          "type": "essay",
          "prompt": `[[equation:15.5]]와 [[equation:15.6]]을 통해 $Y$의 평균과 분산의 이론치를 구하고, 구한 이론치가 문제 2.D1의 시뮬레이션 결과와 일치하는지 확인하시오.`,
          "referenceAnswer": `
$$
Y=X_1+3X_2-2X_3
$$
...
또한, $X_1$, $X_2$, $X_3$는 서로 독립이므로 [[equation:15.6]]에 따라 $Y$의 분산은
$$
\\sigma_Y^2
=
1^2\\times2
...
`
        },
        {
          ...
          "title": "2.E2.",     
          "type": "essay",
          "prompt": `문제 2.E1의 결과와 문제 2.D2에서 [[equation:15.5]]와 [[equation:15.6]]을 이용하여 구한 평균과 분산을 이용하면, 문제 2.D에서 생성한 확률 변수 $Y$(Python 변수=‘Y’)의 정확한 PDF $f_Y(y)$를 구할 수 있다. $f_Y(y)$를 쓰시오.`,
          "referenceAnswer": `
...
`
        },
        {
          ...
          "title": "2.E3.",     
          "type": "console",
          "prompt": `[[equation:15.4]]를 이용하여 문제 2.D에서 생성한 ‘Y’가 10보다 높을 확률 $\\Pr\\{Y \\ge 10\\}$을 구할 수 있다. 아래 명령어를 완성하여 Console 창에서 그 값을 구하고, 결과를 쓰시오.
\`\`\`python
>>> import numpy as np
>>> from scipy.special import erfc
>>> m=?; v=?; t=? #완성해야 할 부분 (1)
    ...
    { //문제 3
      ...
      "problems": [
        { //문제 3.A
          "id": "15-3A",
          "title": "3.A.", 
          "prompt": `$X$가 $[1, 2, 3, ..., 6]$ 값들을 균일한 확률로 갖는 독립 랜덤 변수일 때, [[equation:15.7]]과 같이 정의되는 랜덤 변수 $Y$를 고려하자.
$$
Y=\\sum_{i=1}^{M}X_i
\\qquad \\text{(식 15.7)}
$$
...
\`\`\`
        ...
        },
        {
          ...
          "title": "3.A1.",
          "type": "essay",    
          "prompt": `$M=2$일 때, ‘Y’가 가질 수 있는 가능한 값을 모두 쓰고, [[equation:15.7]]에서 랜덤 변수 $Y$가 가질 수 있는 최솟값이 $M$일 때, $Y$가 가질 수 있는 최댓값을 $M$에 대한 식으로 나타내시오.`,
...
`
        ...
        { //문제 4.C
          "id": "15-4C",
          "title": "4.C.",
          "prompt": `랜덤 프로세스의 자기 상관 함수를 실험으로 구하고 관찰하자. 랜덤 프로세스 $x(t)$의 자기 상관 함수 $R_x(t_1, t_2)$는 [[equation:15.8]]과 같이 나타낼 수 있다.
$$
R_x(t_1,t_2)
...
`
        },
        {
          ...
          "title": "4.D2.",
          "type": "essay",
          "prompt": `(a) [[equation:15.8]]의 상관 식에 $\\tau=0$($t_1=t_2$와 같은 의미)을 대입하여 정리하면, 그 값은 신호의 무엇을 의미하는지 쓰시오.
...
},
{
  ...
  "title": "4.E1.",
  "type": "essay",
  "prompt": `[[equation:15.9]]의 $x(t)=a(t)\\cos(\\omega_c t)$에 대하여 평균 $E[x(t)]$를 구하고, 평균이 시간 $t$에 따라 변하는지 판단하시오.`,
  "referenceAnswer": `
$a(t)$의 평균을
...
},
{
  ...
  "title": "4.E2.",
  "type": "essay",
  "prompt": `[[equation:15.9]]의 $x(t)=a(t)\\cos(\\omega_c t)$에 대하여 자기 상관 함수 $R_x(t_1,t_2)$를 구하고, 자기 상관 함수가 시간 차이 $t_2-t_1$에만 의존하는지 판단하시오.`,
  "referenceAnswer": `
자기 상관 함수는
...
따라서 [[equation:15.9]]의 $x(t)$는 일반적으로 WSS가 아니다.
`
},
{
  "id": "15-4E3",
  "title": "4.E3.",
  "type": "essay",
  "prompt": `[[equation:15.10]]의 $x(t)=a(t)\\cos(\\omega_c t+\\theta)$에 대하여 $\\theta$가 $[0,2\\pi]$에서 균일하게 분포할 때, 평균 $E[x(t)]$를 구하시오.`,
  "referenceAnswer": `
$$
E[x(t)]
=
...
},
{
  ...
  "title": "4.E4.",
  "type": "essay",
  "prompt": `[[equation:15.10]]의 자기 상관 함수 $R_x(t_1,t_2)$를 구하고, 그 결과를 이용하여 $x(t)$가 WSS인지 판단하시오.`,
  "referenceAnswer": `
자기 상관 함수는
...
따라서 [[equation:15.10]]의 $x(t)$는 WSS이다.
`
},
        {
            "id": "15-4E5",
            "title": "4.E5.",
            "type": "python",
            "prompt": `[[equation:15.9]]에 해당하는 $x(t)$의 샘플 벡터 ‘xt’를 생성하여 문제 4.E1~4.E2의 답을 검증하자.
...
`
        },
        {
            ...
            "title": "4.E7.",
            "type": "python",
            "prompt": `[[equation:15.10]]에 해당하는 $x(t)$의 샘플 벡터 ‘xt’를 생성하여 문제 4.E3~4.E4의 답을 검증하자.
...
`
        ...
        "title": "4.E9.",
        "type": "essay",
        "prompt": `문제 4.E5~4.E8의 실험 결과가 앞에서 [[equation:15.9]]와 [[equation:15.10]]에 대하여 수식으로 판단한 WSS 여부와 일치하는지 쓰시오.`,
        "referenceAnswer": `
...
x(t)=a(t)\\cos(\\omega_c t)
$$
의 경우, 실험에서 $t_1$을 변경하면 자기 상관 함수의 그래프가 달라지는 것을 확인하였다. 따라서 자기 상관 함수가 시간 차이 $\\tau$에만 의존하지 않으므로 WSS가 아니라고 판단하였다. 이는 앞의 수식 유도에서 [[equation:15.9]]의 랜덤 프로세스가 일반적으로 WSS가 아니라는 결과와 일치한다.

반면 [[equation:15.10]]의
$$
x(t)=a(t)\\cos(\\omega_c t+\\theta)
$$
의 경우, 랜덤 위상 $\\theta$를 추가한 후에는 $t_1$을 변경하여도 자기 상관 함수의 전체적인 형태가 거의 동일하게 나타났다. 또한 평균도 시간에 관계없이 일정하게 나타나므로 WSS라고 판단하였다. 이 역시 앞의 수식 유도에서 [[equation:15.10]]의 랜덤 프로세스가 WSS라는 결과와 일치한다.
