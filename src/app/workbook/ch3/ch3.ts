...
\`\`\``
        ...
        { //문제 2.E
          ...
          "title": "2.E.",
          "type": "python",
          "prompt": `문제 2.D의 과정을 참고하여 , $x$축을 $\\omega$로 하고 $y$축을 $\\angle H(\\omega)$로 하여, $\\omega=[-120, 120]$ 범위에서 $\\angle H(\\omega)$를 그리는 py 스크립트를 작성하시오. 이후, 완성한 py 스크립트를 실행하여 결과 그래프를 확인하시오.`,
          referenceAnswer: `
\`\`\`python
import numpy as np
import matplotlib.pyplot as plt
...
\`\`\``
  ...
  title: "2.F.",
  type: "essay",
  prompt: `문제 2.D~2.E의 수행 결과를 바탕으로 이 회로를 저역통과필터(Low Pass Filter)라 부르는 이유를 아래의 물음을 통해 알아보자.

(a) LPF인지 판단하기 위해서는, 입력주파수에 따른 출력의 진폭을 봐야 하는지, 입력주파수에 따른 출력의 위상을 봐야 하는지?
(b) 왜 그런지? (즉, (a) 답의 이유)
(c) 문제 2.D, 2.E 결과에서 판단에 필요한 결과만 언급해서 LPF인지 최종 판단하시오.`,
  referenceAnswer: `(a) 입력주파수에 따른 출력의 **진폭**, 즉 주파수 전달함수의 크기 $|H(\\omega)|$를 확인해야 한다.

(b) 저역통과필터(LPF)는 낮은 주파수 성분은 잘 통과시키고 높은 주파수 성분은 감쇠시키는 필터이다. 따라서 LPF인지 판단하려면 주파수에 따라 출력 신호의 진폭이 어떻게 달라지는지 확인해야 한다. 위상 응답 $\\angle H(\\omega)$만으로는 주파수별 진폭의 감쇠 정도를 판단할 수 없다.

(c) 문제 2.D에서 얻은 주파수 전달함수의 크기는
$$
|H(\\omega)|
=
\\frac{1}{\\sqrt{1+(\\omega RC)^2}}
...
$\\omega=0$에서 $|H(0)|=1$로 최댓값을 가지며, $|\\omega|$가 증가할수록 $|H(\\omega)|$는 감소한다.

따라서 이 회로는 낮은 주파수 성분은 잘 통과시키고 높은 주파수 성분은 감쇠시키므로 **저역통과필터(LPF)**라고 판단할 수 있다.

이 판단에는 문제 2.D의 진폭 응답만 필요하며, 문제 2.E의 위상 응답은 필요하지 않다.`
},
      ],
    },
    { //문제 3
