...
`
        ...
        { //문제 1.B
          "id": "2-1B",
          "title": "1.B.",
          "prompt": `[[equation:2.1]]의 값을 두 가지 방법으로 구해보자.
$$
...
`       
        },
        {
          ...
          "title": "1.B2.",
          "type": "python",
          "prompt": `[[link:/workbook/ch1?p=1-5A|1장의 문제 5]]를 참고하여, 위 적분식의 값을 sympy 모듈을 사용하여 구하는 py 스크립트를 작성하고, 실행하여 결과를 확인하시오.`,
          "referenceAnswer":  `
예시 코드는 다음과 같다.
\`\`\`python
import sympy as sp
...
for n in range(1,N+1):
    ...
    plt.grid()`,
        ...
        },
        {
          ...
          "title": "2.B5",
          "type": "essay",
          "prompt": `문제 2.B2에서 확인한 오디오 신호를 임의의 시간 지점에서 $T$초 길이만큼 잘라낸 신호를 $f(t)$라 하자. 이제, 직교 신호 집합 $\\{s_1(t), s_2(t), s_3(t), \\dots \\}$의 원소들을 이용하여 $f(t) \\simeq \\sum_{n=1}^{N} f_n s_n(t)$로 근사화하려 한다. $f(t)$와 $\\sum_{n=1}^{N} f_n s_n(t)$의 오차를 최소화하는 $f_n$의 수식은 [[equation:2.2]]와 같이 $f(t)$와 $s_n(t)$를 이용하여 결정됨을 상기하자.
$$
f_n
=
\\frac{
...
plt.plot(t,ft_approx,'r',label='$ft_{approx}$')
plt.legend()`,
          "referenceAnswer": `[[equation:2.2]]에서 $f(t)$는 \`ft\`, $s_n(t)$는 \`snt\`에 대응하므로 네 군데 ?에는 차례대로 \`ft\`, \`snt\`, \`snt\`, \`snt\`가 들어간다.
...
형태의 직교 신호들을 사용할 수 있으므로, $N$을 증가시킬수록 주어진 신호를 더 잘 근사할 수 있다. 따라서, 문제 2.D1에서 사용한 직교 집합은 orthogonal하지만 complete하지 않다고 판단할 수 있다.`            
        ...
        { //문제 2.E
          "id": "2-2E",
          "title": "2.E",
          "prompt": `$t_1 \\le t \\le t_2$ 범위에서 문제 2.B에서 주어진 $f(t)$의 지수(exponetial) 푸리에 급수(Fourier Series) $F_n$을 수치적분을 이용하여 구해보자. $F_n$은 [[equation:2.3]]과 같이 표현할 수 있고, 여기서 $f(t)$의 주기는 $T=t_2-t_1$이다.
$$
F_n
=
\\frac{
...
$$
        ...
        },
        {
          ...
          "title": "2.E1",
          "type": "essay",
          "prompt": `[[equation:2.3]]의 지수 푸리에 급수에서 기본 주파수 $\\omega_0$를 $t_1$, $t_2$의 함수로 나타내고, 이를 이용하여 지수함수 집합이 $t_1 \\le t \\le t_2$ 구간에서 서로 직교함을 확인해보자. 다음 증명의 빈칸 ①~⑤를 채우시오.
...
이므로 지수함수 집합은 $t_1\\le t\\le t_2$ 구간에서 서로 직교한다.`
        },
        {
          ...
          "title": "2.E2",
          "type": "python",
          "prompt": `아래는 [[equation:2.3]]을 수치적분으로 구하고, $0 \\le t \\le T$ 범위에서 $f(t)$, $\\sum_{n=-3}^{3} F_n e^{jn\\omega_0 t}$를 그리는 py 스크립트이다. 11번째 라인의 'X'를 자신의 학번 끝 한자리로 바꾼 후, py 스크립트를 실행하고 결과 그래프를 확인하시오.
\`\`\`python
import numpy as np
import matplotlib.pyplot as plt
from scipy.io import loadmat
...
\\omega_0=\\frac{2\\pi}{t_2-t_1}
$$
로 설정하고, [[equation:2.3]]을 이용하여 $F_n$을 계산한 뒤
$$
