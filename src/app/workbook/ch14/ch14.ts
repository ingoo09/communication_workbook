...
랜덤 변수 X의 평균의 실험치는 [[equation:14.1]]과 같이 나타낼 수 있다. 여기서,  $X[k]$는 $X$의 $k$번째 성분으로 정의된다.
$$
...
N이 충분히 크면 실험적으로 계산한 m은 이론적인 평균 0에 가까운 값을 갖는다. 다만, 랜덤 샘플을 유한한 개수만 사용하므로 정확히 0이 되지 않을 수 있다. N을 10배 증가시키면 일반적으로 표본 평균의 변동이 감소하여 0에 더 가까워진다.`
        },
        {
          ...
          "type": "console",
          "prompt": `  
랜덤 변수 X의 분산의 실험치는 [[equation:14.2]]과 같이 나타낼 수 있다.
$$
v=
\\left(
...
이를 위해, 문제 4.B의 'z' 생성문을 'z=?*np.random.randn(N)+?'로 수정하여 'N'개의 독립적인 랜덤 변수를 생성하고, [[equation:14.1]]과 [[equation:14.2]]를 Python에서 구현하여 'z'의 평균과 분산을 계산하시오.
...
평균의 정의인 [[equation:14.3]]에 따라 수치적분으로 'x'의 평균을 구할 수 있다.
$$
...
`,
        ...
        },
        {
          ...
          "title": "6.B.",
          "type": "python",
          "prompt": `분산의 정의인 [[equation:14.4]]에 따라 수치적분으로 'x'의 분산을 구할 수 있다.
$$
\\sigma_X^2
\\triangleq
...
f_X(x)\\,dx
$$
이를 풀어쓰면, 아래와 같이 분산이 [[equation:14.5]]와 같음을 알 수 있다.
$$
\\sigma_X^2
=
\\int
...
가 된다.`
        },
        {
          ...
          "title": "6.C2.",
          "type": "python",
          "prompt": `[[equation:14.5]]에 따라 수치적분으로 'x'의 분산을 구할 수 있다.
...
\`\`\`
[[equation:14.5]] σ_X² = E[X²] - m_X²에서 E[X²]는 ∫ x² f_X(x) dx이므로 수치적분에서는 sum(x**2*Px)*x_step으로 근사할 수 있다.
...
최종적으로 E[α²] = 2σ²`
        },
        {
          "id": "14-7B",
          "title": "7.B.",
          "prompt": `[[equation:14.8]]과 같이 두 개의 독립적인 가우시안 랜덤 변수를 사용하여, 레일 레이 랜덤 변수 $\\alpha$를 만들 수 있다. 여기서, $z$와 $y$는 평균이 0이고 분산이 $\\sigma^2$인 독립 가우시안 랜덤 변수이다.
$$
\\alpha = \\sqrt{z^2+y^2}
\\qquad \\text{(식 14.8)}
$$
        ...
        },
        {
          ...
          "title": "7.B1.",
          "type": "console",
          "prompt": `$\\sigma^2=1$일 때, [[equation:14.8]]에 따라 $\\alpha$의 샘플을 생성해보자. 함수 'np.random.randn()'을 이용하여, $\\alpha$의 샘플 'alpha_sample'을 생성하는 코드를 완성하시오.
...
문제 3.A에서 완성한 py 스크립트 파일에, 문제 7.B1의 답을 활용하여 py 스크립트 파일을 적절히 수정하고, [[equation:14.7]]을 참고하여 이론적인 PDF는 파란색으로 겹쳐 그리도록 작성하자. 요구사항에 맞게 py 스크립트를 수정한 후 실행하여 결과 그림을 확인하시오.
